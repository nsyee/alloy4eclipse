...
public class AlloyMessageConsole extends MessageConsole {
	...
	/**
	 * Add an listener which matchs the filename from Err in the console, and
	 * transform it in a Hyperlink.
	 * */
	...
	protected void print(final String message, final Color c, final int style) {
		...
		// this.
		if (display != null)// demande a display d'executer le print (dans un
							// thread graphique)
	...
	protected void addFileLink(String filename, int offset, int lenght, int line) {
		if (filename != null) {
			IFile iff = ResourcesPlugin.getWorkspace().getRoot()
